...
namespace FarmingEngine
{
    ...
    public class TheGame : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (gdata.start_day_time + 0.5f < gdata.auto_sleep_time)
            {
                if (pdata.day_time > gdata.auto_sleep_time)
                    TransitionToNextDay();
            }
            else if (gdata.start_day_time > gdata.auto_sleep_time + 0.5f)
            {
                if (pdata.day_time < gdata.start_day_time && pdata.day_time > gdata.auto_sleep_time)
                    TransitionToNextDay();
            }
        }
